...
/**
 * Internal primitives needed for an Owl exchange.
 * <p>
 * These operations are package-private implementation details of
 * {@link OwlClient} / {@link OwlServer} (and the registration drivers
 * {@link OwlClientRegistration} / {@link OwlServerRegistration}). They are not
 * part of the public Owl API and may change without notice; callers should drive
 * exchanges through those classes rather than against these primitives directly.
 */
class OwlUtil
{
     ...
     * @return x2 * pi mod n
     */
    static BigInteger calculateX2s(
        BigInteger n,
        BigInteger x2,
        BigInteger pi)
    {
     ...
     */
    static ECSchnorrZKP calculateZeroknowledgeProof(
        ECPoint generator,
        BigInteger n,
        BigInteger x,
        ECPoint X,
    ...
    static void validateZeroknowledgeProof(
        ECPoint generator,
        ECPoint X,
        ECSchnorrZKP zkp,
        BigInteger q,
     ...
     */
    static void validateParticipantIdsDiffer(
    ...
    /**
     * Calculates the keying material, which can be done after {@link OwlClient#authenticationFinish(OwlAuthenticationServerResponse)} has completed
     * for client and when {@link OwlServer#authenticationServerEnd(OwlAuthenticationFinish)} has completed for server.
     * A session key must be derived from this key material using a secure key derivation function (KDF).
     * The KDF used to derive the key is handled externally (i.e. not by {@link OwlServer} or {@link OwlClient}).
     * <pre>
     ...
     */
    static ECPoint calculateKeyingMaterial(
        ECPoint gx4,
        BigInteger x2,
        BigInteger x2pi,
        ECPoint B)
     ...
     */
    static void validatePublicKey(ECPoint X, ECCurve curve, BigInteger q, BigInteger coFactor) throws CryptoException
    {
     ...
     */
    static BigInteger calculateMacTag(
        String participantId,
