 ...
 * An instance of {@link OwlServerRegistration} represents one server, and
 * an instance of {@link OwlClientRegistration} represents one client.
 * These together make up the main machine through which user registration is facilitated.
 * <p>
 * To execute the registration, construct an {@link OwlServerRegistration} on the server end,
 * and construct an {@link OwlClientRegistration} on the client end.
 * Each Owl registration will need a new and distinct {@link OwlServerRegistration} and {@link OwlClientRegistration}.
 * You cannot use the same {@link OwlServerRegistration} or {@link OwlClientRegistration} for multiple exchanges.
 * <p>
 * For user login go to {@link OwlClient} and {@link OwlServer}.
 * To execute the user registration phase, both
 * {@link OwlServerRegistration} and {@link OwlClientRegistration} must be constructed. 
 * <p>
 * The following communication between {@link OwlServerRegistration} and {@link OwlClientRegistration}, must be
 ...
 * <ul>
 * <li> {@link OwlClientRegistration#initiateUserRegistration()} - The client sends payload to the server over a secure channel. </li>
 * <li> {@link OwlServerRegistration#registerUseronServer(OwlInitialRegistration)} - The server uses the payload received from the client to calculate a user credential payload that is to be safely stored on the server.</li>
 * </ul>
 * <p>
...
public class OwlClientRegistration
{
    ...
    /**
     * Convenience constructor for a new {@link OwlClientRegistration} that uses
    ...
    /**
     * Construct a new {@link OwlClientRegistration}.
     * <p>
        ...
        SecureRandom random)
    {
        ...
        OwlUtil.validateNotNull(password, "password");
     ...
     * <p>
     * Must be called prior to {@link OwlServerRegistration#registerUseronServer(OwlInitialRegistration)}
     * @return {@link OwlInitialRegistration}
    ...
    public OwlInitialRegistration initiateUserRegistration()
    {
        ...
        return new OwlInitialRegistration(clientId, pi, gt);
    }
    ...
    private BigInteger calculatePi()
    {
        try 
        {
            return OwlUtil.calculatePi(n, t, digest);
        }
        catch (CryptoException e)
        {
            throw Exceptions.illegalStateException(e.getMessage(), e);
