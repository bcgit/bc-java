    ...
    implements AsymmetricBlockCipher
{
     ...
     * @throws InvalidCipherTextException
     */
	private byte[] decodeBlockOrRandom(byte[] in, int inOff, int inLen, int pLen)
			throws InvalidCipherTextException {
		if (!forPrivateKey) {
		 ...
		 * Check the padding.
		 */
		int correct = PKCS1Encoding.checkPkcs1Encoding(block, pLen);
        ...
        throws InvalidCipherTextException
    {
		 ...
		 */
		if (this.pLen != -1) {
    		return this.decodeBlockOrRandom(in, inOff, inLen, this.pLen);
    	}
