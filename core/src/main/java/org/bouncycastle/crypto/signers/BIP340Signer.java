    ...
    implements Signer
{
    ...
    private static byte[] sign(ECPrivateKeyParameters privateKey, byte[] m, byte[] auxRand)
    {
        ...
        // Step 1-2: d' = int(sk); fail if d' = 0 or d' >= n.
        ...
        // Steps 5-7: t = bytes(d) XOR H_aux(a); k' = int(H_nonce(t || bytes(P) || m)) mod n; fail if k' = 0.
        ...
        // Steps 8-9: R = k' * G; k = k' if has_even_y(R) else n - k'.
        ...
        // Steps 10-11: e = int(H_challenge(bytes(R) || bytes(P) || m)) mod n; sig = bytes(R) || bytes((k + e*d) mod n).
