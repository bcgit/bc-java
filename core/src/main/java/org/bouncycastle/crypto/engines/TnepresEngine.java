    ...
    extends SerpentEngineBase
{
        ...
        int     outOff)
    {
        ...
        Pack.intToBigEndian(wKey[131] ^ X[3], output, outOff);
        Pack.intToBigEndian(wKey[130] ^ X[2], output, outOff + 4);
        Pack.intToBigEndian(wKey[129] ^ X[1], output, outOff + 8);
        Pack.intToBigEndian(wKey[128] ^ X[0], output, outOff + 12);
    }
        ...
        int     outOff)
    {
        ...
        X[3] = wKey[131] ^ Pack.bigEndianToInt(input, inOff);
        X[2] = wKey[130] ^ Pack.bigEndianToInt(input, inOff + 4);
        X[1] = wKey[129] ^ Pack.bigEndianToInt(input, inOff + 8);
        X[0] = wKey[128] ^ Pack.bigEndianToInt(input, inOff + 12);
        ...
        Pack.intToBigEndian(X[3] ^ wKey[3], output, outOff);
