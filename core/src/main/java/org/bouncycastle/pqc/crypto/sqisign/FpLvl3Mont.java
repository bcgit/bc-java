...
/**
 * Level-3 wrapper around {@link FpMontHelper}'s limb-array CIOS Montgomery
 * arithmetic, specialised to the lvl3 prime {@code p = 65 * 2^376 - 1}
 * (383-bit). Provides {@code mulMont}, {@code sqrMont}, {@code toMont},
 * {@code fromMont}, plus modular {@code addModP} / {@code subModP} over
 * little-endian 32-bit limb arrays of length {@link #N} = 12.
 *
 * <p>Inputs to {@link #mulMont} are in Montgomery form ({@code a*R mod p}
 * with {@code R = 2^{32N} = 2^384}); the output is in Montgomery form,
 * fully reduced to {@code [0, p)}.</p>
 *
 * <p>Verified against a BigInteger oracle.</p>
 */
final class FpLvl3Mont
{
    /**
     * Number of 32-bit little-endian limbs. {@code 12 × 32 = 384 ≥ 383} bits.
     */
    ...
    {
        P_LIMBS = new int[N];
        FpMontHelper.toLimbs(N, FpLvl3.P, P_LIMBS);
        BigInteger R = BigInteger.ONE.shiftLeft(32 * N);
        R_SQR_MOD_P_LIMBS = new int[N];
        FpMontHelper.toLimbs(N, R.multiply(R).mod(FpLvl3.P), R_SQR_MOD_P_LIMBS);
        ONE_MONT_LIMBS = new int[N];
        FpMontHelper.toLimbs(N, R.mod(FpLvl3.P), ONE_MONT_LIMBS);
