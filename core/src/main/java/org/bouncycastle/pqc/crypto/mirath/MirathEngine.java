...
class MirathEngine
{
    ...
    public void mirathMatrixExpandSeedPublicMatrix(byte[] H, byte[] seedPk)
    {
        ...
        prng.doFinal(H, 0, mirathMatrixFFBytesSize(eA, k));
    ...
    private void parse(byte[] input, byte[] output, int offPtr, int nRowsBytes)
    {
    ...
    private int parse(byte[] output, byte[] input, int offPtr, int numColumns, int nRowsBytes, int onCol)
    {
    ...
    private byte mirathMatrixFFGetEntry(byte[] matrix, int off, int nRows, int i, int j)
    {
        ...
        if (isA)
        {
            int pos = j * bytesPerCol + (i >>> 1) + off;
            return (byte)((i & 1) != 0 ? (matrix[pos] >>> 4) & 0x0F : matrix[pos] & 0x0F);
        }
        else
        {
            return (byte)((matrix[off + bytesPerCol * j + (i >>> 3)] >>> (i & 7)) & 0x01);
        }
    }
    ...
    {
        System.arraycopy(S, 0, aux, 0, ffSBytes);
        System.arraycopy(C, 0, aux, ffSBytes, ffCBytes);
        for (int i = 0; i < n1; i++)
        {
            mirathExpandShare(cipher, sample, salt, seeds[mirathTcithPsi(i, e)]);
            // Update base matrices with finite field operations
            byte phi_i = (byte)i;
            mirathMatrixFFMuAddMultipleFF(SBase, phi_i, sample, m, r);
            mirathMatrixFFMuAddMultipleFF(CBase, phi_i, sample, ffSBytes, r, m - r);
            mirathVectorFFMuAddMultiple(vBase, phi_i, sample, ffSBytes + ffCBytes, rho);

            // Performs S_acc = S_acc + S_rnd, C_acc = C_acc + C_rnd and v[e] = v[e] + v_rnd
            Bytes.xorTo(ffSBytes, sample, aux);
            Bytes.xorTo(ffCBytes, sample, ffSBytes, aux, ffSBytes);
            Bytes.xorTo(rho, sample, ffSBytes + ffCBytes, v);
        }
    }
                                       ...
                                       byte[] salt, byte[] S, byte[] C, byte[][] seeds, byte[] sample, short[] v_rnd)
    {
        ...
        for (int i = 0; i < n1; i++)
        {
            ...
            // Update base matrices with finite field operations
            short phi_i = (short)i;
            mirathMatrixFFMuAddMultipleFF(S_base, phi_i, sample, m, r);
            mirathMatrixFFMuAddMultipleFF(C_base, phi_i, sample, ffSBytes, r, m - r);
            mirathVectorFFMuAddMultiple(v_base, phi_i, v_rnd, rho);

            // Performs S_acc = S_acc + S_rnd, C_acc = C_acc + C_rnd and v[e] = v[e] + v_rnd
            Bytes.xorTo(ffSBytes, sample, aux);
            Bytes.xorTo(ffCBytes, sample, ffSBytes, aux, ffSBytes);
            Shorts.xorTo(rho, v_rnd, v);
        }
    }

    private void parseV(byte[] sample, short[] v_rnd)
    {
    ...
    private void mirathExpandShare(BlockCipher cipher, byte[] sample, byte[] salt, byte[] seed)
    {
        int sampleOff = 0;
        ...
        mirathMatrixSetToFF(sample, m, r);
        mirathMatrixSetToFF(sample, ffSBytes, r, m - r);
    }
    ...
    void computeFinalHash(SHA3Digest digest, byte[] hSh, byte[] salt, byte[] hCom, byte[][] aux)
    {
    ...
    public void mirathMatrixFFMuAddMultipleFF(byte[] matrix, byte scalar, byte[] src, int nRows, int nCols)
    {
        if (isA)
        {
            for (int i = 0; i < nRows; i++)
            {
                ...
                {
                    matrix[j * nRows + i] ^= mirathFFMuMult(scalar, MIRATH_MAP_FF_TO_FF_MU[mirathMatrixFFGetEntry(src, nRows, i, j)]);
                }
            }
        }
        else
        {
            for (int i = 0; i < nRows; i++)
            {
                for (int j = 0; j < nCols; j++)
                {
                    matrix[j * nRows + i] ^= mirathFFMuMult(scalar, (byte)(mirathMatrixFFGetEntry(src, nRows, i, j) & 0x01));
                }
            }
        }
    }
    ...
    public void mirathMatrixFFMuAddMultipleFF(byte[] matrix, byte scalar, byte[] src, int off, int nRows, int nCols)
    {
        if (isA)
        {
            for (int i = 0; i < nRows; i++)
            {
                for (int j = 0; j < nCols; j++)
                {
                    matrix[j * nRows + i] ^= mirathFFMuMult(scalar, MIRATH_MAP_FF_TO_FF_MU[mirathMatrixFFGetEntry(src, off, nRows, i, j)]);
                }
            }
        }
        else
        {
            for (int i = 0; i < nRows; i++)
            {
                for (int j = 0; j < nCols; j++)
                {
                    matrix[j * nRows + i] ^= mirathFFMuMult(scalar, (byte)(mirathMatrixFFGetEntry(src, off, nRows, i, j) & 0x01));
                }
            }
        }
    }

    private void matrixFFMuAddMu1FFTo(byte[] matrix1, byte[] matrix3, int rows, int cols)
    {
        if (isA)
        {
            for (int i = 0; i < rows; i++)
            {
                ...
                {
                    matrix1[j * rows + i] ^= MIRATH_MAP_FF_TO_FF_MU[mirathMatrixFFGetEntry(matrix3, rows, i, j)];
                }
            }
        }
        else
        {
            for (int i = 0; i < rows; i++)
            {
                for (int j = 0; j < cols; j++)
                {
                    matrix1[j * rows + i] ^= (byte)(mirathMatrixFFGetEntry(matrix3, rows, i, j) & 0x01);
                }
            }
        }
    }

    private void matrixFFMuAddMu1FFTo(short[] matrix1, byte[] matrix3, int rows, int cols)
    {
        if (isA)
        {
            for (int i = 0; i < rows; i++)
            {
                for (int j = 0; j < cols; j++)
                {
                    matrix1[j * rows + i] ^= mirathMatrixFFGetEntry(matrix3, rows, i, j);
                }
            }
        }
        else
        {
            for (int i = 0; i < rows; i++)
            {
                for (int j = 0; j < cols; j++)
                {
                    matrix1[j * rows + i] ^= mirathMatrixFFGetEntry(matrix3, rows, i, j) & 0x01;
                }
            }
        }
    }
    ...
    public void mirathMatrixFFMuAddMultipleFF(short[] matrix, short scalar, byte[] src, int nRows, int nCols)
    {
        for (int i = 0; i < nRows; i++)
        {
            for (int j = 0; j < nCols; j++)
            {
                matrix[nRows * j + i] ^= mirathFFMuMult(scalar, mirathMatrixFFGetEntry(src, nRows, i, j));
            }
        }
    }

    public void mirathMatrixFFMuAddMultipleFF(short[] matrix, short scalar, byte[] src, int off, int nRows, int nCols)
    {
        for (int i = 0; i < nRows; i++)
        {
            for (int j = 0; j < nCols; j++)
            {
                matrix[nRows * j + i] ^= mirathFFMuMult(scalar, mirathMatrixFFGetEntry(src, off, nRows, i, j));
            }
        }
    }
    ...
    public void mirathVectorFFMuAddMultipleFF(byte[] vector1, byte scalar, byte[] vector3, int ncols)
    {
        for (int i = 0; i < ncols; i++)
        {
            ...
            // GF(2^8) multiplication followed by addition (XOR)
            vector1[i] ^= mirathFFMuMult(scalar, MIRATH_MAP_FF_TO_FF_MU[t]);
        }
    }
    ...
    public void mirathVectorFFMuAddMultipleFF(short[] vector1, short scalar, byte[] vector3, int ncols)
    {
        for (int i = 0; i < ncols; i++)
        {
            byte t = mirathMatrixFFGetEntry(vector3, 1, i, 0);
            // GF(2^8) multiplication followed by addition (XOR)
            vector1[i] ^= mirathFFMuMult(scalar, t);
        }
    }
    ...
    public void emulateMPCMu(byte[] baseAlpha, byte[] midAlpha, byte[] S, byte[] S_rnd, byte[] C, byte[] C_rnd,
                             byte[] v, byte[] rnd_v, byte[] gamma, byte[] H)
    {
        // Temporary storage
        byte[] e_A = new byte[eA]; // m * m - k
        byte[] e_B = new byte[k]; // k
        byte[] aux_s = new byte[s]; // m * r
        byte[] aux_E = new byte[baseMid]; // m * (m - r)
        // 7. Intermediate calculations
        byte[] aux_c = new byte[c];
        byte[] sc = new byte[mirathMatrixFFBytesSize(m, m - r)];
        // 1. aux_E = S_rnd * C_rnd
        matrixFFMuProduct(aux_E, S_rnd, C_rnd, m, r, m - r);

        // 2. Split codeword
        splitCodewordFFMu(e_A, e_B, aux_s, aux_E);

        // 3. e_A + (H * e_B)
        matrixFFMuProductFF1MuTo(e_A, H, e_B, eA, k);

        // 5-6. gamma * [e_A + (H * e_B)] + rnd_V
        matrixFFMuProductXor(baseAlpha, gamma, e_A, rnd_v);
        ...
        matrixFFMuProductTo(aux_E, aux_s, aux_c, m, r, m - r);

        // 12. sc = S * C
        matrixFFProduct(sc, S, C, m, r, m - r);

        // 13. aux_E = aux_E + sc
        matrixFFMuAddMu1FFTo(aux_E, sc, m, m - r);
        ...
        matrixFFMuProductXor(midAlpha, gamma, e_A, v);
    }

    public void emulateMPCMu(short[] baseAlpha, short[] midAlpha, byte[] S, short[] S_rnd, byte[] C, short[] C_rnd,
                             short[] v, short[] rnd_v, short[] gamma, byte[] H)
    {
        // Temporary storage
    ...
    public void matrixFFMuProduct(byte[] result, byte[] matrix1, byte[] matrix2, int nRows1, int nCols1, int nCols2)
    {
        for (int i = 0; i < nRows1; i++)
        {
            for (int j = 0; j < nCols2; j++)
            {
                result[nRows1 * j + i] = getEntry_i_j(matrix1, matrix2, nRows1, nCols1, i, j);
            }
        }
    }
    ...
    private byte getEntry_i_j(byte[] matrix1, byte[] matrix2, int nRows1, int nCols1, int i, int j)
    {
        ...
        for (int k = 0; k < nCols1; k++)
        {
            byte entry_i_k = matrix1[nRows1 * k + i];//mirathMatrixFFMuGetEntry(matrix1, nRows1, i, k);
            byte entry_k_j = matrix2[nCols1 * j + k];//mirathMatrixFFMuGetEntry(matrix2, nCols1, k, j);
            entry_i_j ^= mirathFFMuMult(entry_i_k, entry_k_j);
        }
        return entry_i_j;
    }

    public void matrixFFMuProductTo(byte[] result, byte[] matrix1, byte[] matrix2, int nRows1, int nCols1, int nCols2)
    {
        for (int i = 0; i < nRows1; i++)
        {
            for (int j = 0; j < nCols2; j++)
            {
                byte entry_i_j = getEntry_i_j(matrix1, matrix2, nRows1, nCols1, i, j);
                result[j * nRows1 + i] ^= entry_i_j;
            }
        }
    }
    ...
    public void matrixFFMuProduct(short[] result, short[] matrix1, short[] matrix2, int nRows1, int nCols1, int nCols2)
    {
        for (int i = 0; i < nRows1; i++)
        {
            for (int j = 0; j < nCols2; j++)
            {
                result[j * nRows1 + i] = getEntry_i_j(matrix1, matrix2, nRows1, nCols1, i, j);
            }
        }
    }

    public void matrixFFMuProductTo(short[] result, short[] matrix1, short[] matrix2, int nRows1, int nCols1, int nCols2)
    {
        for (int i = 0; i < nRows1; i++)
        {
            for (int j = 0; j < nCols2; j++)
    ...
    private void matrixFFMuProductFF1MuTo(byte[] result, byte[] mat1, byte[] mat2, int rows1, int cols1)
    {
        if (isA)
        {
            for (int i = 0; i < rows1; i++)
            {
                ...
                for (int k = 0; k < cols1; k++)
                {
                    ...
                    byte entry_k_j = mat2[k];//mirathMatrixFFMuGetEntry(mat2, cols1, k, 0);
                    entry_i_j ^= mirathFFMuMult(entry_i_k, entry_k_j);
                }
                result[i] ^= entry_i_j;
            }
    ...
    private void matrixFFMuProductFF1MuTo(short[] result, byte[] mat1, short[] mat2, int rows1, int cols1)
    {
        if (isA)
        {
            for (int i = 0; i < rows1; i++)
        ...
        else
        {
            for (int i = 0; i < rows1; i++)
            {
                ...
                for (int k = 0; k < cols1; k++)
                {
                    entry_i_j ^= mirathFFMuMult((short)(mirathMatrixFFGetEntry(mat1, rows1, i, k) & 0x01), mat2[k]);
                }
                result[i] ^= entry_i_j;
            }
        }
    ...
    private void matrixFFMuAddMu1FF(byte[] matrix1, byte[] matrix2, byte[] matrix3, int rows, int cols)
    {
        if (isA)
        {
            for (int i = 0; i < rows; i++)
            {
                for (int j = 0; j < cols; j++)
                {
                    byte entry1 = matrix2[rows * j + i];//mirathMatrixFFMuGetEntry(matrix2, rows, i, j);
    ...
    private void matrixFFMuAddMu1FF(short[] matrix1, short[] matrix2, byte[] matrix3, int rows, int cols)
    {
        ...
        else
        {
            for (int i = 0; i < rows; i++)
            {
                ...
                {
                    short entry1 = matrix2[rows * j + i];
                    short entry2 = (short)(mirathMatrixFFGetEntry(matrix3, rows, i, j) & 0x01);
                    matrix1[rows * j + i] = (short)(entry1 ^ entry2);
                }
            }
        }
    ...
    public void mirathTcithHashMpc(SHA3Digest digest, byte[] hMpc, byte[] pk, byte[] salt, byte[] msg, byte[] hSh,
    ...
    void expandViewChallenge(int[] challenge, byte[] vGrinding, byte[] input)
    {
        ...
        for (int e = 0; e < tau; e++)
        {
            ...
            {
                for (int i = 0; i < challenge2Bytes - 1; i++)
                {
                    random[i] = (byte)(((random[i] & 0xff) >>> 1) ^ ((random[i + 1] & 0xff) << 7));
                }
                random[challenge2Bytes - 1] = (byte)((random[challenge2Bytes - 1] & 0xff) >>> 1);
            }
        }
    }
    ...
    public long mirathTcithOpenRandomShare(byte[][] path, byte[][] commitsIStar, byte[][] tree, byte[][][] commits,
                                           byte[] binding)
    {
                                 ...
                                 byte[][] aux, byte[][] midAlpha)
    {
        ...
        for (int e = 0; e < tau; e++)
        {
            ...
            offPtr = parse(signature, aux[e], offPtr, r, nRowsBytes1, onCol1);

            // Process (N-R) columns (R x (N-R) matrix)
            offPtr = parse(signature, aux[e], offPtr, m - r, nRowsBytes2, onCol2);
    ...
    {
        // Copy salt
        System.arraycopy(salt, 0, signature, 0, saltBytes);
        ptr = saltBytes;
    ...
    private int parseSignatureToAux(byte[] aux, byte[] signature, int offPtr, int loop, int nRowsBytes, int onCol)
    {
        ...
        {
            aux[col] = (byte)((signature[ptr] & 0xff) >>> (8 - offPtr));
            for (int i = 0; i < nRowsBytes - 1; ++i)
            {
                ptr++;
                aux[col] |= (byte)((signature[ptr] & 0xff) << offPtr);
                col++;
                aux[col] = (byte)((signature[ptr] & 0xff) >>> (8 - offPtr));
            }
            if (offPtr <= onCol)
    ...
    public void parseSignature(int pos, byte[][] aux, byte[][] midAlpha, byte[] signature)
    {
        ...
        for (int e = 0; e < tau; e++)
        {
            ...
            for (int i = 0; i < rho; i++)
            {
                byte entry = (byte)((signature[ptr] & 0xff) >>> (8 - offPtr));
                ptr++;
                      ...
                      byte[] salt, byte[] sample)
    {
        ...
        for (int i = 0; i < n1; i++)
        {
            if (i != i_star)
            {
                ...
                mirathMatrixFFMuAddMultipleFF(S_share, sc, sample, m, r);
                mirathMatrixFFMuAddMultipleFF(C_share, sc, sample, ffSBytes, r, m - r);
                mirathVectorFFMuAddMultiple(v_share, sc, sample, ffSBytes + ffCBytes, rho);
            }
        }
                      ...
                      byte[] salt, byte[] sample, short[] vi)
    {
        ...
        for (int i = 0; i < n1; i++)
        {
            if (i != i_star)
            {
                ...
                mirathMatrixFFMuAddMultipleFF(S_share, sc, sample, m, r);
                mirathMatrixFFMuAddMultipleFF(C_share, sc, sample, ffSBytes, r, m - r);
                mirathVectorFFMuAddMultiple(v_share, sc, vi, rho);
            }
        }
                               ...
                               byte[] gamma, byte[] H, byte[] y, byte[] midAlpha)
    {
        ...
        mirathMatrixFFMuAddMultiple2(Ts, (byte)p, S_share);
        ...
        mirathVectorFFMuAddMultipleFF(e_A, pSquared, y, eA);

        // gamma * tmp -> baseAlpha
        matrixFFMuProductXor(baseAlpha, gamma, e_A, v_share);

        // Add mid_alpha * p
        mirathVectorFFMuAddMultiple(baseAlpha, (byte)p, midAlpha, rho);
    }
                               ...
                               short[] gamma, byte[] H, byte[] y, short[] midAlpha)
    {
        ...
        // p * S_share (accumulated in Ts)
        mirathMatrixFFMuAddMultiple2(Ts, (short)p, S_share);
        ...
        // Add mid_alpha * p
        mirathVectorFFMuAddMultiple(baseAlpha, (short)p, midAlpha, rho);
    }
