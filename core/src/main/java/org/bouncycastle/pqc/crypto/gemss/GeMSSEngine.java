...
//        }
    ...
    int genSecretMQS_gf2_opt(Pointer MQS, Pointer F)
    {
        ...
        for (k = 1; k < HFEn; ++k)
        {
            ...
            {
                int vec_x_orig = a_vec_kp.getIndex();
                int vec_y_orig = buf_k.getIndex();
                ...
                buf_k.changeIndex(vec_y_orig);
                a_vec_k.changeIndex(vec2_x_orig);
                buf_kp.changeIndex(vec2_y_orig + HFEDegI * NB_WORD_GFqn);
                MQS.move(NB_WORD_GFqn);
            }
            /* Vinegar variables */
            F_cp.changeIndex(F_lin);
    ...
    void mul_gf2n(Pointer P, Pointer A, int AOff, Pointer B)
    {
        ...
        A.move(AOff);
        //TODO 128 versions REM192_SPECIALIZED_TRINOMIAL_GF2X with ki64
        //TODO gemss256 384 version for REM384_SPECIALIZED_TRINOMIAL_GF2X with ki64
        //TODO bluegemss256, redgemss256 REM384_SPECIALIZED358_TRINOMIAL_GF2X with ki64
        //TODO whitegemss256, cyangemss256, magentagemss256 REM384_TRINOMIAL_GF2X with ki64
    ...
    private long convMQ_uncompressL_gf2(Pointer pk2, PointerUnion pk)
    {
        ...
        nb_bits = 1;
        nb_bits = for_setpk2_end_move_plus(pk2, pk64, nb_bits, HFEnvq);
    ...
    private long convMQ_last_uncompressL_gf2(Pointer pk2, PointerUnion pk)
    {
        ...
        nb_bits = for_setpk2_end_move_plus(pk2, pk64, nb_bits, HFEnvqm1);
    ...
    private int for_setpk2_end_move_plus(Pointer pk2, PointerUnion pk64, int nb_bits, int len)
    {
        /* For each row */
...
//        }
        ...
        /* Compute H1 = H(m) */
        PointerBuffer Hi = new PointerBuffer(Hi_tab, Sha3BitStrength >>> 3);
        SHA3Digest sha3Digest = new SHA3Digest(Sha3BitStrength);
        sha3Digest.update(m, m_cp, len);
        ...
        for (k = 1; k <= NB_ITE; ++k)
        {
            ...
            do
            {
                ...
                if (HFEmr8 != 0)//except fgemss and dualmodegs
                {
                    ...
                    DR_cp.setAndByte(HFEmq8, -(1L << HFEmr8));
                    /* Store rem_char */
                ...
                if (chooseRootHFE_gf2nx(DR, sk_HFE.F_struct, U) == 0)
                {
...
//                }
    ...
    {
        Pointer L, U;
        sk_HFE.sk_uncomp = new Pointer(NB_UINT_HFEVPOLY + (LTRIANGULAR_NV_SIZE << 1) + (LTRIANGULAR_N_SIZE << 1) + MATRIXnv_SIZE + MATRIXn_SIZE);
        SHAKEDigest shakeDigest = new SHAKEDigest(ShakeBitStrength);
        shakeDigest.update(sk, 0, SIZE_SEED_SK);
        byte[] sk_uncomp_byte = new byte[(NB_UINT_HFEVPOLY + (LTRIANGULAR_NV_SIZE << 1) + (LTRIANGULAR_N_SIZE << 1)) << 3];//<< 3
        shakeDigest.doFinal(sk_uncomp_byte, 0, sk_uncomp_byte.length);
        sk_HFE.sk_uncomp.fill(0, sk_uncomp_byte, 0, sk_uncomp_byte.length);
        sk_HFE.S = new Pointer(sk_HFE.sk_uncomp, NB_UINT_HFEVPOLY + (LTRIANGULAR_NV_SIZE << 1) + (LTRIANGULAR_N_SIZE << 1));
    ...
    private void for_mul(Pointer res, Pointer inv, int start)
    {
        for (int i = start; i != -1; --i)
        {
            mul_gf2n(res, i * NB_WORD_GFqn, res, i * NB_WORD_GFqn, inv);
        }
    }
    ...
    private int getD_for_not0_or_plus(Pointer Xqn)
    {
        int i, d;
        long mask, b;
    ...
    private void divsqr_r_HFE_cst_gf2nx(Pointer poly, SecretKeyHFE.complete_sparse_monic_gf2nx F, Pointer cst)
    {
        ...
        if (ENABLED_REMOVE_ODD_DEGREE)
        {
            ...
            {
                mul_xorrange_loop(poly, cst, F, dp);
            }
    ...
    private void mul_xorrange_loop(Pointer poly, Pointer cst, SecretKeyHFE.complete_sparse_monic_gf2nx F, int dp)
    {
        Pointer leading_coef = new Pointer(poly, dp * NB_WORD_GFqn);
        Pointer res = new Pointer(leading_coef, -HFEDeg * NB_WORD_GFqn);
        /* i=0: Constant of F-U */
    ...
    private void convHFEpolynomialSparseToDense_gf2nx(Pointer F_dense, SecretKeyHFE.complete_sparse_monic_gf2nx F)
    {
        ...
        /* i=0: constant of F */
        F_dense.copyFrom(F.poly, NB_WORD_GFqn);
        for_copy_move(F_dense, F);
    ...
    void fast_sort_gf2n(Pointer tab, int l)
    {
        ...
        int i, quo, rem, pow2_prev, pa, pb, pc;
    ...
    public void compress_signHFE(byte[] sm8, Pointer sm)
    {
        ...
        System.arraycopy(sm64, 0, sm8, 0, NB_BYTES_GFqnv);
        /* Take the (Delta+v)*(nb_ite-1) bits */
        int k1, nb_bits, nb_rem2, nb_rem_m, val_n, nb_rem;
    ...
    void convMQS_one_eq_to_hybrid_rep8_uncomp_gf2(byte[] pk, PointerUnion pk_cp)
    {
        ...
        for (k = 2, i = 2; i < HFEnv; ++i)
        {
            k = setPK(pk, pk_U, i, pk2_cp, pk_U_cp, k, HFEnv - 1, HFEnv - i);
        }
        /* i == HFEnv */
        nb_bits = HFEnv;
        /* For each column */
    ...
    public int crypto_sign_open(byte[] PK, byte[] message, byte[] signature)
    {
        ...
        {
            return sign_openHFE_uncomp_pk(message, message.length, signature, pk);
        }
    }
    ...
    private void LOOPIR_INIT(Pointer MQS2_cp, Pointer MQS_cpj, Pointer MQS_cpi, Pointer S_cpj, int STARTIR, int NB_ITIR)
    {
        for (int ir = STARTIR; ir < NB_ITIR; ++ir)
        {
            ...
            MQS_cpj.changeIndex(MQS_cpi);
            /* Compute a dot product */
            //TODO for removal, LOOPKR(MQS_cpj, MQS2_cp, S_cpj.get(kq), 0, NB_BITS_UINT) expand with MQS2_cp.setRangeAndMaskMove(MQS_cpj, NB_WORD_GFqn, -(bit_kr & 1L));
