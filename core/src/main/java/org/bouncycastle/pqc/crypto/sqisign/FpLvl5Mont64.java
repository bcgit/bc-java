...
/**
 * Level-5 wrapper around {@link FpMontHelper64}'s 64-bit limb CIOS kernel.
 * Lvl5 prime {@code p = 27 * 2^500 - 1} (505-bit) → 8 limbs of 64 bits = 512 bits.
 */
final class FpLvl5Mont64
{
    ...
    {
        P_LIMBS = new long[N];
        FpMontHelper64.toLimbs(N, FpLvl5.P, P_LIMBS);
        BigInteger R = BigInteger.ONE.shiftLeft(64 * N);
        R_SQR_MOD_P_LIMBS = new long[N];
        FpMontHelper64.toLimbs(N, R.multiply(R).mod(FpLvl5.P), R_SQR_MOD_P_LIMBS);
        ONE_MONT_LIMBS = new long[N];
        FpMontHelper64.toLimbs(N, R.mod(FpLvl5.P), ONE_MONT_LIMBS);
        CANONICAL_ONE_LIMBS = new long[N];
        CANONICAL_ONE_LIMBS[0] = 1L;
        BIAS_2P2 = FpMontHelper64.bias2P2(N, FpLvl5.P);
    }

    private FpLvl5Mont64()
    {
    }
