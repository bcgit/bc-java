    ...
    extends SimpleTest
{
    ...
    private void shouldRejectDER(String label, ASN1Primitive prim)
    {
        ...
        {
            // NOTE: jdk1.3 overlay. Throwable.getCause() is a Java 1.4 API absent on JDK 1.3, and
            // the jdk1.3 Exceptions.ioException(msg, cause) overlay drops the cause entirely (no
            // 1.3 caller can observe it), so the DEREncodingException-as-cause assertion the base
            // test makes is unprovable here -- the message-content assertion below is the only
            // part of this check 1.3 can still make.
            isTrue("unexpected message rejecting " + label,
