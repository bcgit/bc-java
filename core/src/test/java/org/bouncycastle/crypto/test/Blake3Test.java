package org.bouncycastle.crypto.test;

import org.bouncycastle.crypto.digests.Blake3Digest;
import org.bouncycastle.crypto.params.Blake3Parameters;
import org.bouncycastle.util.Arrays;
import org.bouncycastle.util.encoders.Hex;
import org.bouncycastle.util.test.SimpleTest;

/**
 * Test Cases for Blake3.
 * Test Vectors taken from https://github.com/BLAKE3-team/BLAKE3/blob/master/test_vectors/test_vectors.json
 */
public class Blake3Test
    extends SimpleTest
{
    /**
     * The Blake3 Data Modulus.
     */
    private static final int BLAKE3MOD = 251;

    /**
     * The partial output size.
     */
    private static final int PARTOUT = 5;

    /**
     * The partial break position.
     */
    private static final int PARTBREAK = 100;

    /**
     * The Blake3 Test key.
     */
    private static final byte[] BLAKE3KEY = "whats the Elvish word for friend".getBytes();

    /**
     * The Blake3 Test context.
     */
    private static final byte[] BLAKE3CTX = "BLAKE3 2019-12-27 16:29:52 test vectors context".getBytes();

    private final TestDef TEST0 = new TestDef(0,
        "",
        "",
        ""
    );
    private final TestDef TEST1 = new TestDef(1,
        "",
        "",
        ""
    );
    private final TestDef TEST2 = new TestDef(2,
        "",
        "",
        ""
    );
    private final TestDef TEST3 = new TestDef(3,
        "",
        "",
        ""
    );
    private final TestDef TEST4 = new TestDef(4,
        "",
        "",
        ""
    );
    private final TestDef TEST5 = new TestDef(5,
        "",
        "",
        ""
    );
    private final TestDef TEST6 = new TestDef(6,
        "",
        "",
        ""
    );
    private final TestDef TEST7 = new TestDef(7,
        "",
        "",
        ""
    );
    private final TestDef TEST8 = new TestDef(8,
        "",
        "",
        ""
    );
    private final TestDef TEST63 = new TestDef(63,
        "",
        "",
        ""
    );
    private final TestDef TEST64 = new TestDef(64,
        "",
        "",
        ""
    );
    private final TestDef TEST65 = new TestDef(65,
        "",
        "",
        ""
    );
    private final TestDef TEST127 = new TestDef(127,
        "",
        "",
        ""
    );
    private final TestDef TEST128 = new TestDef(128,
        "",
        "",
        ""
    );
    private final TestDef TEST129 = new TestDef(129,
        "",
        "",
        ""
    );
    private final TestDef TEST1023 = new TestDef(1023,
        "",
        "",
        ""
    );
    private final TestDef TEST1024 = new TestDef(1024,
        "",
        "",
        ""
    );
    private final TestDef TEST1025 = new TestDef(1025,
        "",
        "",
        ""
    );
    private final TestDef TEST2048 = new TestDef(2048,
        "",
        "",
        ""
    );
    private final TestDef TEST2049 = new TestDef(2049,
        "",
        "",
        ""
    );
    private final TestDef TEST3072 = new TestDef(3072,
        "",
        "",
        ""
    );
    private final TestDef TEST3073 = new TestDef(3073,
        "",
        "",
        ""
    );
    private final TestDef TEST4096 = new TestDef(4096,
        "",
        "",
        ""
    );
    private final TestDef TEST4097 = new TestDef(4097,
        "",
        "",
        ""
    );
    private final TestDef TEST5120 = new TestDef(5120,
        "",
        "",
        ""
    );
    private final TestDef TEST5121 = new TestDef(5121,
        "",
        "",
        ""
    );
    private final TestDef TEST6144 = new TestDef(6144,
        "",
        "",
        ""
    );
    private final TestDef TEST6145 = new TestDef(6145,
        "",
        "",
        ""
    );
    private final TestDef TEST7168 = new TestDef(7168,
        "",
        "",
        ""
    );
    private final TestDef TEST7169 = new TestDef(7169,
        "",
        "",
        ""
    );
    private final TestDef TEST8192 = new TestDef(8192,
        "",
        "",
        ""
    );
    private final TestDef TEST8193 = new TestDef(8193,
        "",
        "",
        ""
    );
    private final TestDef TEST16384 = new TestDef(16384,
        "",
        "",
        ""
    );
    private final TestDef TEST31744 = new TestDef(31744,
        "",
        "",
        ""
    );
    private final TestDef TEST102400 = new TestDef(102400,
        "",
        "",
        ""
    );

    public String getName()
    {
        return "Blake3";
    }

    public void performTest()
        throws Exception
    {
        runTestCase(TEST0);
        runTestCase(TEST1);
        runTestCase(TEST2);
        runTestCase(TEST3);
        runTestCase(TEST4);
        runTestCase(TEST5);
        runTestCase(TEST6);
        runTestCase(TEST7);
        runTestCase(TEST8);
        runTestCase(TEST63);
        runTestCase(TEST64);
        runTestCase(TEST65);
        runTestCase(TEST127);
        runTestCase(TEST128);
        runTestCase(TEST129);
        runTestCase(TEST1023);
        runTestCase(TEST1024);
        runTestCase(TEST1025);
        runTestCase(TEST2048);
        runTestCase(TEST2049);
        runTestCase(TEST3072);
        runTestCase(TEST3073);
        runTestCase(TEST4096);
        runTestCase(TEST4097);
        runTestCase(TEST5120);
        runTestCase(TEST5121);
        runTestCase(TEST6144);
        runTestCase(TEST6145);
        runTestCase(TEST7168);
        runTestCase(TEST7169);
        runTestCase(TEST8192);
        runTestCase(TEST8193);
        runTestCase(TEST16384);
        runTestCase(TEST31744);
        runTestCase(TEST102400);
    }

    /**
     * Create the blockCipher test suite.
     *
     * @return the test stream
     */
    private void runTestCase(final TestDef pTest)
    {
        /* run tests */
        testHash(pTest);
        testKeyed(pTest);
        testDerived(pTest);
    }

    /**
     * Run hashTest.
     *
     * @param pTest the test
     */
    private void testHash(final TestDef pTest)
    {
        /* Create digest */
        final Blake3Digest myDigest = new Blake3Digest();

        /* Try a fake update */
        myDigest.update(BLAKE3KEY, 0, BLAKE3KEY.length);
        myDigest.reset();

        /* Save just before final and reset */
        myDigest.update(pTest.theData, 0, pTest.theLength);
        final Blake3Digest mySave = (Blake3Digest)myDigest.copy();
        myDigest.reset();

        /* restore save and perform a standard hash */
        myDigest.reset(mySave);
        final byte[] myBase = new byte[myDigest.getDigestSize()];
        myDigest.doFinal(myBase, 0);
        final byte[] myExpected = Arrays.copyOf(pTest.theHash, myBase.length);
        isTrue("baseDigest mismatch", Arrays.areEqual(myExpected, myBase));

        /* Perform Xof hash */
        myDigest.update(pTest.theData, 0, pTest.theLength);
        final byte[] myOutput = new byte[pTest.theHash.length];
        checkMultiOutput(myDigest, myOutput);
        isTrue("digest mismatch", Arrays.areEqual(pTest.theHash, myOutput));
    }

    /**
     * Run keyedTest.
     *
     * @param pTest the test
     */
    private void testKeyed(final TestDef pTest)
    {
        /* Create digest */
        final Blake3Digest myDigest = new Blake3Digest();
        final Blake3Parameters myParams = Blake3Parameters.key(BLAKE3KEY);
        myDigest.init(myParams);

        /* Try a fake update */
        myDigest.update(BLAKE3KEY, 0, BLAKE3KEY.length);
        myDigest.reset();

        /* Save just before final and reset */
        myDigest.update(pTest.theData, 0, pTest.theLength);
        final Blake3Digest mySave = (Blake3Digest)myDigest.copy();
        myDigest.reset();

        /* restore save and perform a standard hash */
        myDigest.reset(mySave);
        final byte[] myBase = new byte[myDigest.getDigestSize()];
        myDigest.doFinal(myBase, 0);
        final byte[] myExpected = Arrays.copyOf(pTest.theKeyed, myBase.length);
        isTrue("baseKeyedDigest mismatch", Arrays.areEqual(myExpected, myBase));

        /* Perform Xof hash */
        myDigest.update(pTest.theData, 0, pTest.theLength);
        final byte[] myOutput = new byte[pTest.theKeyed.length];
        checkMultiOutput(myDigest, myOutput);
        isTrue("keyedDigest mismatch", Arrays.areEqual(pTest.theKeyed, myOutput));
    }

    /**
     * Run derivedTest.
     *
     * @param pTest the test
     */
    private void testDerived(final TestDef pTest)
    {
        /* Create digest */
        final Blake3Digest myDigest = new Blake3Digest();
        final Blake3Parameters myParams = Blake3Parameters.context(BLAKE3CTX);
        myDigest.init(myParams);

        /* Try a fake update */
        myDigest.update(BLAKE3KEY, 0, BLAKE3KEY.length);
        myDigest.reset();

        /* Save just before final and reset */
        myDigest.update(pTest.theData, 0, pTest.theLength);
        final Blake3Digest mySave = (Blake3Digest)myDigest.copy();
        myDigest.reset();

        /* restore save and perform a standard hash */
        myDigest.reset(mySave);
        final byte[] myBase = new byte[myDigest.getDigestSize()];
        myDigest.doFinal(myBase, 0);
        final byte[] myExpected = Arrays.copyOf(pTest.theDerived, myBase.length);
        isTrue("baseDerivedDigest mismatch", Arrays.areEqual(myExpected, myBase));

        /* Perform Xof hash */
        myDigest.update(pTest.theData, 0, pTest.theLength);
        final byte[] myOutput = new byte[pTest.theDerived.length];
        checkMultiOutput(myDigest, myOutput);
        isTrue("derivedDigest mismatch", Arrays.areEqual(pTest.theDerived, myOutput));
    }

    /**
     * check multi-output.
     *
     * @param pDigest the digest
     * @param pBuffer the buffer
     */
    private void checkMultiOutput(final Blake3Digest pDigest,
                                  final byte[] pBuffer)
    {
        /* Take a copy of the state */
        final Blake3Digest mySave = (Blake3Digest)pDigest.copy();

        /* Crate alternative buffer */
        final int myLen = pBuffer.length;
        final byte[] myAlt = new byte[myLen];

        /* Perform standard output */
        pDigest.doOutput(pBuffer, 0, myLen);
        pDigest.reset(mySave);

        /* Loop creating output 5 bytes at a time */
        for (int i = 0; i < myLen; i += PARTOUT)
        {
            /* Output bytes */
            int myRead = Math.min(PARTOUT, myLen - i);
            pDigest.doOutput(myAlt, i, myRead);

            /* Around halfway through */
            if (i == PARTBREAK)
            {
                /* Check save and reset in the middle of output */
                final Blake3Digest myTemp = (Blake3Digest)pDigest.copy();
                pDigest.reset();
                pDigest.reset(myTemp);
            }
        }
        pDigest.reset(mySave);
        isTrue("multiOutput mismatch", Arrays.areEqual(pBuffer, myAlt));
    }

    /**
     * Test Definition.
     */
    private static class TestDef
    {
        /**
         * The data length.
         */
        private final int theLength;

        /**
         * The testData.
         */
        private final byte[] theData;

        /**
         * The expected hash.
         */
        private final byte[] theHash;

        /**
         * The expected keyedHash/
         */
        private final byte[] theKeyed;

        /**
         * The expected derived Hash.
         */
        private final byte[] theDerived;

        /**
         * Constructor.
         *
         * @param pLength  the length
         * @param pHash    the expected hash.
         * @param pKeyed   the expected keyed hash
         * @param pDerived the expected derived hash.
         */
        TestDef(final int pLength,
                final String pHash,
                final String pKeyed,
                final String pDerived)
        {
            /* Store parameters */
            theLength = pLength;
            theHash = Hex.decode(pHash);
            theKeyed = Hex.decode(pKeyed);
            theDerived = Hex.decode(pDerived);

            /* Generate test data */
            theData = new byte[theLength];
            for (int i = 0; i < theLength; i++)
            {
                theData[i] = (byte)(i % BLAKE3MOD);
            }
        }
    }

    /**
     * Main entry point.
     *
     * @param args the argyments
     */
    public static void main(String[] args)
    {
        runTest(new Blake3Test());
    }
}
