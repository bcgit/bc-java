    ...
    extends AbstractTlsCrypto
{
    ...
    public boolean hasNamedGroup(int namedGroup)
    {
        ...
        int key = Integers.valueOf(namedGroup).intValue();

        synchronized (supportedGroups)
        {
            Boolean cached = (Boolean)supportedGroups.get(new Integer(key));
            if (cached != null)
            {
        ...
        synchronized (supportedGroups)
        {
            supportedGroups.put(new Integer(key), Boolean.valueOf(result));
        }
