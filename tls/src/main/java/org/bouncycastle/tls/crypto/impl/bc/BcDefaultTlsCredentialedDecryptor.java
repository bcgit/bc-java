    ...
    implements TlsCredentialedDecryptor
{
    ...
    {
        int rsaBytes = (((RSAKeyParameters)privateKey).getModulus().bitLength() + 7) >>> 3;

        RSABlindedEngine rsa = new RSABlindedEngine();
        rsa.init(false, privateKey);
        if (rsa.getOutputBlockSize() > rsaBytes)
        {
            throw new TlsFatalAlert(AlertDescription.internal_error);
        }

        byte[] plaintext = rsa.processBlock(ciphertext, 0, ciphertext.length);

        return TlsImplUtils.unpadPreMasterSecret(crypto, cryptoParams, rsaBytes, plaintext, 0, plaintext.length);
    }
