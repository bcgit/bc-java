...
class CRC
{
    ...
    int getFinal()
    {
        return ~Integers.reverseBytes(value);
    }

    void update(int inCh)
    {
        value = (value >>> 8) ^ CRC32_TABLE[(value ^ inCh) & 0xFF];
    }

    void updateRun(int inCh, int runLength)
    {
//        assert runLength >= 4;

        int inCh2 = inCh << 8 | inCh;
        int inCh4 = inCh2 << 16 | inCh2;
