...
public interface OpenPGPPolicy
{
     ...
     * produced, which is the only point at which decompression can be stopped - a consumer
     * counting bytes off the returned stream has already paid the cost it is trying to avoid.
     * </p><p>
     * Exceeding it surfaces as a {@link org.bouncycastle.util.io.StreamOverflowException} (an
     * {@link java.io.IOException}) from reading the
     * {@link OpenPGPMessageInputStream message input stream}.
     * </p><p>
     ...
     * low-level API: the default is unbounded, and bounding it is the caller's decision.
     * </p>
     *
     * @return maximum decompressed size in bytes, or a negative value for no limit
     */
    long getMaximumDecompressedDataSize();
