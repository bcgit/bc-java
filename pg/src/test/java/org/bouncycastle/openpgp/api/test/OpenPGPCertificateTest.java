    ...
    extends APITest
{
    ...
    {
        testGetEncryptionKeysForAndCondition(api);
    ...
    private void testGetEncryptionKeysForAndCondition(OpenPGPApi api)
        throws IOException, PGPException
    {
        try
        {
            Date creationTime = currentTimeRounded();
            Date evaluationTime = new Date(creationTime.getTime() + 86400000L); // one day later

            // 1. Master key (Ed25519) with self‑signature (DIRECT_KEY)
            KeyPairGenerator masterGen = KeyPairGenerator.getInstance("EDDSA", new BouncyCastleProvider());
            masterGen.initialize(new EdDSAParameterSpec("Ed25519"));
            KeyPair masterKp = masterGen.generateKeyPair();
            JcaPGPKeyPair masterKeyPair = new JcaPGPKeyPair(PublicKeyPacket.VERSION_4, PublicKeyAlgorithmTags.Ed25519, masterKp, creationTime);

            PGPSignatureGenerator masterSigGen = new PGPSignatureGenerator(new JcaPGPContentSignerBuilder(
                masterKeyPair.getPublicKey().getAlgorithm(), HashAlgorithmTags.SHA256)
                .setProvider(new BouncyCastleProvider()));
            masterSigGen.init(PGPSignature.DIRECT_KEY, masterKeyPair.getPrivateKey()); // correct type for primary key
            PGPSignatureSubpacketGenerator masterSub = new PGPSignatureSubpacketGenerator();
            masterSub.setKeyFlags(true, KeyFlags.CERTIFY_OTHER | KeyFlags.SIGN_DATA);
            masterSub.setKeyExpirationTime(true, 0);
            masterSigGen.setHashedSubpackets(masterSub.generate());
            PGPSignature masterSig = masterSigGen.generateCertification(masterKeyPair.getPublicKey());
            PGPPublicKey masterWithSig = PGPPublicKey.addCertification(masterKeyPair.getPublicKey(), masterSig);

            // 2. Subkey (ECDH X25519) with binding signature (SUBKEY_BINDING)
            KeyPairGenerator subGen = KeyPairGenerator.getInstance("X25519", new BouncyCastleProvider());
            KeyPair subKp = subGen.generateKeyPair();
            JcaPGPKeyPair subKeyPair = new JcaPGPKeyPair(PublicKeyPacket.VERSION_4, PublicKeyAlgorithmTags.ECDH, subKp, creationTime);

            PGPSignatureGenerator bindingSigGen = new PGPSignatureGenerator(new JcaPGPContentSignerBuilder(
                masterKeyPair.getPublicKey().getAlgorithm(), HashAlgorithmTags.SHA256)
                .setProvider(new BouncyCastleProvider()));
            bindingSigGen.init(PGPSignature.SUBKEY_BINDING, masterKeyPair.getPrivateKey());
            PGPSignatureSubpacketGenerator bindingSub = new PGPSignatureSubpacketGenerator();
            bindingSub.setKeyFlags(true, KeyFlags.ENCRYPT_COMMS | KeyFlags.ENCRYPT_STORAGE);
            bindingSigGen.setHashedSubpackets(bindingSub.generate());
            PGPSignature bindingSig = bindingSigGen.generateCertification(masterWithSig, subKeyPair.getPublicKey());
...
//        ring.encode(aOut);
//        aOut.close();
//        OpenPGPCertificate cert = api.readKeyOrCertificate().parseCertificate(new String(out.toByteArray()));
            // TODO: Investigate why the subkey is lost when serializing to armored and parsing via api.readKeyOrCertificate().
            // For now, we construct OpenPGPCertificate directly from the ring.
            OpenPGPCertificate cert = new OpenPGPCertificate(ring);

            // 5. Test OR vs. AND
            List orResult = cert.getComponentKeysWithFlag(evaluationTime,
                new int[]{KeyFlags.ENCRYPT_COMMS, KeyFlags.ENCRYPT_STORAGE});
            isEquals(1, orResult.size());

            List andAttempt = cert.getComponentKeysWithFlag(evaluationTime,
                new int[]{KeyFlags.ENCRYPT_COMMS | KeyFlags.ENCRYPT_STORAGE});
            ...
            {
                OpenPGPCertificate.OpenPGPComponentKey key = (OpenPGPCertificate.OpenPGPComponentKey)encryptionKeys.get(i);
                if (key.hasKeyFlags(evaluationTime, KeyFlags.ENCRYPT_COMMS) &&
                    key.hasKeyFlags(evaluationTime, KeyFlags.ENCRYPT_STORAGE))
                {
                    bothFlags.add(key);
                }
            }
            isEquals(1, bothFlags.size());
        }
