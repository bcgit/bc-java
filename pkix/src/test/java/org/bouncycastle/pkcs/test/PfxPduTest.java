...
//                assertEquals(PKCSObjectIdentifiers.certBag, bags[0].getType());
        ...
        throws Exception
    {
        ...
        PKCS12PfxPdu pfx = new PKCS12PfxPdu(Streams.readAll(TestResourceFinder.findTestResource("asn1", "test_key_pbmac1_bcjsse.pfx")));

        assertTrue(pfx.hasMac());
        assertTrue(pfx.isMacValid(new JcePKCS12MacCalculatorBuilderProvider().setProvider("BC"), password));

        scanPfxFile(pfx, inputDecryptorProvider);
    }

    public void testPBMAC1_OpenSSL()
        throws Exception
    {
        char[] password = "xxxxxx".toCharArray();
        ...
        InputDecryptorProvider inputDecryptorProvider = new JcePKCSPBEInputDecryptorProviderBuilder()
            .setProvider("BC").build(password);
        PKCS12PfxPdu pfx = new PKCS12PfxPdu(Streams.readAll(TestResourceFinder.findTestResource("asn1", "test_key_pbmac1_openssl.pfx")));
    ...
    private void scanPfxFile(PKCS12PfxPdu pfx, InputDecryptorProvider inputDecryptorProvider)
        throws PKCSException
    {
        ContentInfo[] infos = pfx.getContentInfos();
        boolean certBag = false;
        boolean pkcs8ShroudedKeyBag = false;
        ...
        for (int i = 0; i != infos.length; i++)
        {
            ...
            else
            {
                ...
                pkcs8ShroudedKeyBag = true;
            }
        }

        assertTrue(certBag && pkcs8ShroudedKeyBag);
    }
