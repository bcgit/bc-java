...
public class JceCMSContentEncryptorBuilder
{
    ...
    // select the desired instance") on the unqualified algorithmIdentifier references below: this
    // class's private algorithmIdentifier field (line 65-ish, outer JceCMSContentEncryptorBuilder)
    // and CMSOutEncryptor's protected algorithmIdentifier field of the same name are both in scope
    // from CMSOutputEncryptor/CMSAuthOutputEncryptor (nested in the outer class AND extending
    // CMSOutEncryptor), which modern javac resolves silently but 1.3's does not. Qualified with
    // this. throughout both subclasses to disambiguate; CMSOutEncryptor's own methods need no
    // change since they have only one candidate (their own inherited field).
        ...
        implements OutputEncryptor
    {
        ...
        {
            return new JceGenericKey(this.algorithmIdentifier, encKey);
        }
    }
        ...
        implements OutputAEADEncryptor
    {
        ...
        public OutputStream getOutputStream(OutputStream dOut)
        {
            ...
            if (kdfAlgorithm != null)
            {
                algId = AlgorithmIdentifier.getInstance(this.algorithmIdentifier.getParameters());
            }
            else
            {
                algId = this.algorithmIdentifier;
            }
        ...
        // NOTE: jdk1.3 overlay. javax.crypto.Cipher.updateAAD is a Java 7 JCE API, so the
        // reflective checkForAEAD() probe (and JceAADStream, which uses it) is meaningless on
        // this JRE -- and JceAADStream.java is itself excluded from the jdk1.3 build (see
        // ant/jdk13.xml's pkix/src/main/java fileset exclude). checkForAEAD()/updateAAD support
        // are dropped rather than ported; getAADStream() always returns null on 1.3, same as the
        // base fallback path for pre-Java-7 JDKs.
