...
// NOTE: jdk1.3 overlay. java.nio.charset.Charset is a Java 1.4 API absent on JDK 1.3, so
// Charset.isSupported(String) is replaced with a manual encode probe (isEncodingSupported below).
// RuntimeException(Throwable) is also a Java 1.4 constructor absent on JDK 1.3.
public class LocalizedMessage
{
    ...
    public String getEntry(String key, Locale loc, TimeZone timezone) throws MissingEntryException
    {
        ...
        try
        {
            ...
            // Pre-JDK 1.6 has no ResourceBundle.Control, so the "do not fall back to the JVM default
            // locale" behaviour (github #2249) is emulated by pinning the default locale to the
            // requested one across the lookup - otherwise an explicit request for e.g. English on a
            // JVM whose default locale is German would resolve to the German bundle. (The main
            // src/main/java version uses Control.getNoFallbackControl, unavailable before JDK 1.6.)
