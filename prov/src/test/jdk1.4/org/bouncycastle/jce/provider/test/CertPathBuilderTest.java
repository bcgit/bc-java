    ...
    extends SimpleTest
{
        ...
        throws Exception
    {
        ...
        // declare the test failed. (JDK 1.4 variant: plain Thread + join in
        // place of java.util.concurrent.)
        final Object[] outcome = new Object[2];   // [0] = result, [1] = thrown exception
        Thread worker = new Thread(new Runnable()
        {
            public void run()
            {
                ...
                {
                    CertPathBuilder builder = CertPathBuilder.getInstance("PKIX", "BC");
                    outcome[0] = builder.build(buildParams);
                }
                catch (Exception e)
                {
                    outcome[1] = e;
        ...
        if (outcome[1] != null)
        {
            throw (Exception)outcome[1];
        }

        long elapsedMs = System.currentTimeMillis() - startMs;

        PKIXCertPathBuilderResult result = (PKIXCertPathBuilderResult)outcome[0];
        CertPath path = result.getCertPath();
        if (path.getCertificates().size() != 2)
        {
