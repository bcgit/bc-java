    ...
    implements PKCSObjectIdentifiers, X509ObjectIdentifiers, BCKeyStore
{
        ...
        NoSuchAlgorithmException, CertificateException
    {
        ...
        if (bcParam.getMacAlgorithm().getAlgorithm().equals(id_PBMAC1))
        {
            ...
            AlgorithmIdentifier authScheme = pbmac1Params.getMessageAuthScheme();

            if (!keyDevFunc.getAlgorithm().equals(PKCSObjectIdentifiers.id_PBKDF2))
            {
                throw new IOException(
                    "No support for KDF parameter of type " + keyDevFunc.getAlgorithm().toString());
            }
        }
    ...
    public void engineStore(OutputStream stream, char[] password)
        throws IOException
    {
        doStore(stream, password, true, true);
    }
        ...
        extends AdaptingKeyStoreSpi
    {
        ...
        {
            super(new BCJcaJceHelper(), new PKCS12PBMAC1KeyStoreSpi(new BCJcaJceHelper(), NISTObjectIdentifiers.id_aes256_CBC, NISTObjectIdentifiers.id_aes128_CBC));
        }
    }
