...
public final class EdDSAEngine extends Signature {
    ...
    protected void engineInitSign(PrivateKey privateKey) throws InvalidKeyException {
        reset();
        if (privateKey instanceof RFC7748PrivateKey) {
            RFC7748PrivateKey privKey = (RFC7748PrivateKey) privateKey;
            key = privKey;
            ...
            throw new InvalidKeyException("cannot identify RFC7748 private key: " + privateKey.getClass());
        }
    }

    private void digestInitSign(RFC7748PrivateKey privKey) {
        // Preparing for hash
        // r = H(h_b,...,h_2b-1,M)
    ...
    protected void engineInitVerify(PublicKey publicKey) throws InvalidKeyException {
        reset();
        if (publicKey instanceof RFC7748PublicKey) {
            key = (RFC7748PublicKey) publicKey;
            ...
            throw new InvalidKeyException("cannot identify RFC7748 public key: " + publicKey.getClass());
        }
    }
    ...
    protected byte[] engineSign() throws SignatureException {
        try {
            ...
            RFC7748PrivateKey privKey = (RFC7748PrivateKey) key;
            digestInitSign(privKey);
        }
    }
    ...
    private byte[] x_engineSign() throws SignatureException {
        ...
        byte[] a = ((RFC7748PrivateKey) key).geta();
        ...
        digest.update(Rbyte);
        digest.update(((RFC7748PrivateKey) key).getAbyte());
        digest.update(message, offset, length);
    ...
    private boolean x_engineVerify(byte[] sigBytes) throws SignatureException {
        ...
        digest.update(((RFC7748PublicKey) key).getAbyte());
        // h = H(Rbar,Abar,M)
        byte[] message;
        int offset, length;
        if (oneShotMode) {
        ...
        GroupElement R = key.getParams().getB().doubleScalarMultiplyVariableTime(
                ((RFC7748PublicKey) key).getNegativeA(), h, Sbyte);
