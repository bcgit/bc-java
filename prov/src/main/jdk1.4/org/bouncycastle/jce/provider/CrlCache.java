...
class CrlCache
{
        ...
        throws IOException, CRLException
    {
        ...
        CacheEntry entry = (CacheEntry)cache.get(distributionPoint);
        if (entry != null)
        {
            crlStore = (PKIXCRLStore)entry.ref.get();
        }
    ...
    private static Collection getCrls(CertificateFactory certFact, URI distributionPoint)
        throws IOException, CRLException
    {
        // NOTE: jdk1.4 overlay. Base sets a connect and a read timeout on the connection here;
        // URLConnection.setConnectTimeout / setReadTimeout are Java 5, so this build has neither.
        // Otherwise keep in step with base - in particular, do not reintroduce a downcast to
        // HttpURLConnection, which is what stopped FTP based CRLs loading (github #1867).
