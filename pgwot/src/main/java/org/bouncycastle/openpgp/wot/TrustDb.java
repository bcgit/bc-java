...
//			return TRUST_UNKNOWN; // TODO maybe we should support other trust models...
	 ...
	 * <p>
	 * Either this method or {@link #updateTrustDb()} must be invoked whenever a new key was added
	 * to the key ring, because the WOT-related code does not keep track of key-ring-changes
	 * ({@link #isTrustDbStale()} does not detect them).
	 * @see #isTrustDbStale()
	 * @see #updateTrustDb()
	 */
	...
	/**
	 * Update the {@code trustdb.gpg} by recalculating all keys' validities, if it is needed.
	 * An update is needed, if the {@linkplain #isTrustDbStale() trust-db is stale}.
	 * @see #updateTrustDb()
	 * @see #isTrustDbStale()
	 */
	 ...
	 * <p>
	 * Either this method or {@link #markTrustDbStale()} must be invoked whenever a new key was added
	 * to the key ring, because the WOT-related code does not keep track of key-ring-changes
	 * ({@link #isTrustDbStale()} does not detect them).
	 * <p>
	 * Inspired by {@code static int validate_keys (int interactive)}. This function was not ported,
	 * because the implementation looked overly complicated. This method here is a re-implementation
	 * from scratch. It still seems to come very closely to the behaviour of GnuPG's original code.
	 ...
	 * struct key_item *klist, u32 curtime, u32 *next_expire)}, but re-implemented from scratch - see {@link #updateTrustDb()}.
	 ...
	 * @param pgpKey the pgp-key to be validated. Must not be <code>null</code>.
	 */
