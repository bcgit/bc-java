 ...
 * <p>
 * An instance of this class is used to read from and write to the {@code trustdb.gpg}, which is
 * usually located in {@code ~/.gnupg/}. If this file does not exist (yet), it is created.
 * The containing directory, however, is not created implicitly!
 * <p>
 * <b>Important:</b> Do <b>not</b> use this class directly, if you don't have good reasons to!
 * Instead, you should use the {@link TrustDb}.
 * <p>
...
public class TrustDbIo implements AutoCloseable, TrustConst {
	...
	protected synchronized long getTrustHashRec() {
		if (trustHashRec == 0) {
			TrustRecord.Version version = getTrustRecord(0, TrustRecord.Version.class);
			assertNotNull("version", version);
	...
	protected synchronized long newRecordNum() throws TrustDbIoException {
		long recordNum;
	...
	public synchronized TrustRecord.Trust getTrustByFingerprint(final byte[] fingerprint) throws TrustDbIoException {
	...
	public synchronized TrustRecord getTrustRecordViaHashTable(long table, byte[] key, TrustRecordMatcher matcher) {
		long hashrec, item;
		int msb;
		int level = 0;
	...
	public synchronized <T extends TrustRecord> T getTrustRecord(final long recordNum, Class<T> expectedTrustRecordClass) throws TrustDbIoException {
	...
	public synchronized void putTrustRecord(final TrustRecord trustRecord) throws TrustDbIoException {
	...
	protected synchronized void writeTrustRecord(final TrustRecord record) throws TrustDbIoException {
		int bufIdx = 0;
		final byte[] buf = new byte[TRUST_RECORD_LEN];
	...
	protected synchronized void updateHashTable(long table, byte[] key, long recordNum) throws TrustDbIoException {
//		TrustRecord lastrec, rec;
		...
		next_level: while (true) {
			...
			if (item == 0) { // Insert a new item into the hash table.
			...
			else { // Must do an update.
				lastHashTable = hashTable; hashTable = null;
				...
				if (rec.getType() == TrustRecordType.HTBL) {
				...
				else if (rec.getType() == TrustRecordType.HLST) { // Extend the list.
					...
					// The following line was added by me, Marco. I think the original GnuPG code missed this: We should start looking
					// for a free entry in the *first* suitable HashList record again, because there might have been sth. dropped.
				...
				else { // Insert a list record.
					if (rec.getType() != TrustRecordType.TRUST)
						throw new IllegalStateException(String.format("hashtbl %d: %d/%d points to an invalid record %d",
					...
					assertNotNull("lastHashTable", lastHashTable).setItem(msb % ITEMS_PER_HTBL_RECORD, hashList.getRecordNum());
	...
	public synchronized void flush() throws TrustDbIoException {
	...
	@Override
	public synchronized void close() throws TrustDbIoException {
		if (closed)
			return;
